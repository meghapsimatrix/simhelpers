...
The table below shows each of the performance criteria, what it measures, its definition, its estimate, and its MCSE formula. The equation for the MCSE of RMSE is derived using [the delta method](https://www.jepusto.com/multivariate-delta-method/), which states that:
...
abs_dat <- tibble(Criterion = c("Bias","Variance","MSE", "RMSE"),
              ...
              Definition = c("$\\text{E}(T) - \\theta$", "$\\text{E}(T - \\text{E}(T))^2$",  "\\text{E}(T - \\theta)^2$", "$\\sqrt{\\text{E}(T - \\theta)^2}$"),
...
rel_dat <- tibble(Criterion = c("Relative Bias","Relative MSE"),
              Definition = c("$\\text{E}(L) / \\lambda$", "$\\text{E}(L - \\lambda)^2/ \\lambda^2$"),
