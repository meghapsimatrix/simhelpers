...
If there are any convergence issues, please make sure to write your estimation function to output `NA` for the value of the performance estimate. The functions in the `simhelpers` package are created to handle missingness in performance estimates and will output the number of iterations, `K`, which will exclude the `NA` values. Note that `K` may differ by condition. 
