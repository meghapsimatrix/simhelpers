...
The equation for the MCSE of RMSE is derived  the jacknife technique, which involves excluding a replicate $j$ and calculating RMSE. The formula for RMSE is:
...
To derive the MCSE for Relative RMSE, we again used the jacknife technique. The formula to calculate RMSE is:
...
Here $\bar{T}_{(j)}$ $S^2_{T(j)}$ and indicate the mean and variance of the estimates leaving replicate $j$ out. These are calculated as specified in above when we described the algebra trick to estimate these values. The MCSE is then calculated as specified in the table below. 
...
rel_dat_var <- tibble(Criterion = c("Relative Bias","Relative MSE", "Relative RMSE"),
              ...
              MCSE = c("$\\frac{1}{K} \\sum_{j=1}^K \\left(RB_{(j)} - RB\\right)^2$", "$\\frac{1}{K} \\sum_{j=1}^K \\left(MSE_{(j)} - MSE\\right)^2$",
                      "$\\frac{1}{K} \\sum_{j=1}^K \\left(RMSE_{(j)} - RMSE\\right)^2$" ))
...
welch_res %>%
  mutate(params = mean_diff) %>%
  group_by(n, mean_diff, method) %>%
  group_modify(~ calc_rbvjk(.x,  estimates = est, var_estimates = var)) %>%
  mutate_if(is.numeric, round, 5) %>%
  kable()
```
