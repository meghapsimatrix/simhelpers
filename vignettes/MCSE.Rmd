...
Here $\bar{T}_{(j)}$ and $S^2_{T(j)}$ and indicate the mean and variance of the estimates leaving replicate $j$ out. Instead of calculating each jackknife estimate, we use algebra tricks to calculate $\bar{T}_{(j)}$ and $S^2_{T(j)}$ as follows:
...
rel_dat <- tibble(Criterion = c("Relative Bias","Relative MSE", "Relative RMSE"),
              ...
              MCSE = c("$\\sqrt{S_T^2 / (K\\theta^2)}$", 
                       "$\\sqrt{\\frac{1}{K\\theta^2}\\left[S_T^4 (k_T - 1) + 4 S_T^3 g_T(\\bar{T} - \\theta) + 4 S_T^2 (\\bar{T} - \\theta)^2\\right]}$", "$\\sqrt{\\frac{1}{K} \\left(RMSE_{(j)} - RMSE)^2\\right)}$"))
