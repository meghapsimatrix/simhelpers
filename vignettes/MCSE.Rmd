...
If there are any convergence issues, please make sure to write your estimation function to output `NA` for the value of the performance estimates for cases that did not converge. The functions in the `simhelpers` package are created to handle missingness in performance estimates and will output the number of iterations, `K`, which will exclude the `NA` values. Note that `K` may differ by condition. 
...
If there any convergence issues with estimation of the variance estimates, please again make sure that the estimation function returns `NA` value for the variance estimate. The `calc_relative_var()` will omit that whole row before calculating the performance measure and MCSE. 
