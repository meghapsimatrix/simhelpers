...
abs_dat <- tibble(Criterion = c("Bias","Variance","MSE", "RMSE"),
              ...
              MCSE = c("$\\sqrt{S_T^2/ K}$", "$S_T^2 \\sqrt{\\frac{k_T - 1}{K}}$",  
                       "$\\sqrt{\\frac{1}{K}\\left[S_T^4 (k_T - 1) + 4 S_T^3 g_T(\\bar{T} - \\theta) + 4 S_T^2 (\\bar{T} - \\theta)^2\\right]}$ ", "$\\sqrt{\\frac{1}{K} \\left(RMSE_{(j)} - RMSE)^2\\right)}$"))
...
The equation for the MCSE of RMSE is derived using the jacknife technique, which involves excluding a replicate $j$ and calculating RMSE. The formula for RMSE is:
...
Here $\bar{T}_{(j)}$ and $S^2_{T(j)}$ and indicate the mean and variance of the estimates leaving replicate $j$ out. Instead of calculating each jacknife estimate, we use algebra tricks to calculate $\bar{T}_{(j)}$ and $S^2_{T(j)}$ as follows:
...
welch_res %>%
  ...
  mutate_if(is.numeric, round, 5) %>% # rounding
...
To derive the MCSE for relative RMSE, we again used the jacknife technique. The formula to calculate relative RMSE is:
...
rel_dat <- tibble(Criterion = c("Relative Bias","Relative MSE", "Relative RMSE"),
              ...
              MCSE = c("$\\sqrt{S_T^2 / (K\\theta^2)}$", 
                       "$\\sqrt{\\frac{1}{K\\theta^2}\\left[S_T^4 (k_T - 1) + 4 S_T^3 g_T(\\bar{T} - \\theta) + 4 S_L^2 (\\bar{T} - \\theta)^2\\right]}$", "$\\sqrt{\\frac{1}{K} \\left(RMSE_{(j)} - RMSE)^2\\right)}$"))
...
Similary, we can estimate the MCSE of relative MSE and RMSE using jacknnife technique. To estimate the MSE we need to estimate $S_{V(j)}^2$ which represents the sample variance of the variance estimates leaving replicate $j$ out. We can estimte $S_{V(j)}^2$ as specified below: 

$$S_{V(j)}^2 = \frac{1}{K - 2} \left[(K - 1) S_V^2 - \frac{K}{K - 1}\left(V_j - \bar{V}\right)^2\right]$$

We can then estimate jacknife relative MSE and RMSE following the same logic as we described above and then calculate the MCSE. 
...
Table 4 below presents the performance criteria used to evaluate hypothesis tests. 
...
Below we calculate the rejection rates for the hypothesis tests conducted using t-test. The null hypothesis states that the two groups have the same means. The `calc_rr()` function requires a data frame or tibble containing simulation results as the first argument. The second argument, `p_values`, requires the name of the column containing p-values. The third argument, `alpha` lets the user specify a value for $\alpha$. The default value is set to the conventional 0.05.   
