...
abs_dat <- tibble(Criterion = c("Bias","Variance", "Standard Error", "MSE", "RMSE"),
              Measure = c("Difference from true parameter", "Precision", "Precision", "Accuracy", "Accuracy"),
              Definition = c("$\\text{E}(T) - \\theta$", 
                             "$\\text{E}\\left[(T - \\text{E}(T))^2\\right]$",
                             "$\\text{E}\\left[(T - \\text{E}(T))\\right]$",
                             "$\\text{E}\\left[(T - \\theta)^2\\right]$",    "$\\sqrt{\\text{E}\\left[(T - \\theta)^2\\right]}$"),
              Estimate = c("$\\bar{T} - \\theta$", 
                           "$S_T^2$", 
                           "$S_T$", 
                           "$\\frac{1}{K}\\sum_{k=1}^{K}\\left(T_k - \\theta\\right)^2$", "$\\sqrt{\\frac{1}{K}\\sum_{k=1}^{K}\\left(T_k - \\theta\\right)^2}$"),
              MCSE = c("$\\sqrt{S_T^2/ K}$",
                       "$S_T^2 \\sqrt{\\frac{k_T - 1}{K}}$",  
