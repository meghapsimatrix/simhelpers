...
Below we calculate the maximum Monte Carlo standard error (MCSE) and maximum error rate for the combination of conditions presented in the graph above. We want to examine whether MCSE values are relatively small compared to the estimated Type 1 error rates. The maximum MCSEs are relatively low for the eigen-decomposition methods. However, the maximum MCSEs are close to the naximum estimated error rates for the Hotelling's $T^2$ based methods. 

```{r}
Tipton_Pusto %>%
  group_by(num_studies, q, test) %>%
  summarize(max_mcse = max(mcse),
            max_rej = max(rej_rate)) %>%
  pivot_longer(cols = max_mcse:max_rej,
               names_to = "max",
               values_to = "val") %>%
  unite(test, test, max) %>%
