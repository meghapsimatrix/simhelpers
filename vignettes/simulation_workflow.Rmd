...
The flowchart below depicts the workflow. The chart is created using `grViz()` from [`DiagrammeR`](https://rich-iannone.github.io/DiagrammeR/) package.
...
generate_dat <- function(n, mean_diff){
  ...
  dat <- tibble(group_1 = rnorm(n = n, mean_diff, 1), # mean mean_diff and sd 1 
                group_2 = rnorm(n = n, 0, 2)) # mean 0, sd 2
...
```{r}
(t_res <- bind_rows(tidy(t.test(example_dat$group_1, example_dat$group_2, var.equal = TRUE)), 
...
Here we run the simulation using `purrr` package serial workflow. We use [`pmap()`](https://purrr.tidyverse.org/reference/map2.html) function from `purrr` to run the `run_sim()` function on each condition specified in `params`. 
