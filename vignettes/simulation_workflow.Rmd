...
The following flowchart depicts the workflow. The chart is created using `grViz()` from [`DiagrammeR`](https://rich-iannone.github.io/DiagrammeR/) package.
...
generate_dat <- function(n, mean_diff){
  ...
  dat <- tibble(group_1 = rnorm(n = n, 0, 1), # mean 0 and sd of 1 
                group_2 = rnorm(n = n, mean_diff, 2)) # mean set to mean_diff, sd of 2
...
After creating the data generating function, we should check whether it works. Below, we generate an example dataset with 10,000 people in each group and the `mean_diff` set to 1. We pivot the data to a long format for visualization. 

```{r}
set.seed(2020143)
example_dat <- generate_dat(n = 10000, mean_diff = 1) 

graph_dat <- example_dat %>%
  pivot_longer(cols = c(group_1:group_2),
               names_to = "group",
               values_to = "val") %>%
  mutate(group = if_else(group == "group_1", "Group 1", "Group 2"))

graph_dat %>%
  head()
```

Below, we create a summary table. The mean of the outcome for Group 1 is close to 0 and the mean for Group 2 is close to 1. The standard deviation of the outcome for Group 1 is close to 1 and the standard deviation for Group 2 is close to 2. The table matches what we specified in the data generating model.  
...
ggplot(graph_dat, aes(x = val, fill = group)) + 
...
Here, we run some statistical methods to estimate test statistics, regression coefficients, p-values, or confidence intervals. The function takes in the data and any design parameters. 
...
Here again, it would be good to check if the function is running as it should. Below we run the `estimate()` function on the example dataset: 

```{r}
(res_func <- estimate(example_dat) %>%
...
```{r}
(t_func_res <- bind_rows(tidy(t.test(example_dat$group_1, example_dat$group_2, var.equal = TRUE)), 
          tidy(t.test(example_dat$group_1, example_dat$group_2))) %>%
...
Here we run the `calc_performance()` function on the results we obtained above:

```{r}
calc_performance(res_func)
```
...
Once the cluster is configured, we can just replace [`pmap()`](https://purrr.tidyverse.org/reference/map2.html) from `purrr` with [`future_pmap()`](https://davisvaughan.github.io/furrr/reference/future_map2.html) to run the simulation in parallel. 
...
The `create_skeleton()` function from our `simhelpers` package will open an untitled .R file which will contain the skeleton of functions required to run a simulation study. 
