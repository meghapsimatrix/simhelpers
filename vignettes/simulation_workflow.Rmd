...
Simulation typically involve the following workflow (Morris, White & Crowther, 2018): 
...
generate_dat <- function(n1, n2, mean_diff) {
  ...
  dat <- tibble(
    y = c(rnorm(n = n1, mean_diff, 1), rnorm(n = n2, 0, 2)),
...
In this step, we run some statistical methods to calculate test statistics, regression coefficients, p-values, or confidence intervals. The function takes in the data and any design parameters, such as options
...
estimate <- function(dat, n1, n2){
  ...
  results <- bind_rows(
    calc_t(est = est, vd = vdt, df = dft, method = "t-test"),
