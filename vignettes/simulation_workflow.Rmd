...
Below is an example where random normal data are generated for two groups. The function takes in two arguments, `mean_diff`, indicating the mean difference, and `n`, indicating sample size per group assuming equal sample size. 
...
The code below specifies two design factors `mean_diff` which denotes the mean difference between two groups on an outcome and `n` which specifies whether the sample size. These are the between simulation factors. The within simulation iteration factor is the t-test method with one assuming equal variance and one not assuming equal variance. 
...
Below we use the `future` and the `furrr` package to run the simulation in parallel. These packages are designed to work with the `purrr` functions. The line below sets up a cluster set up on your computer or network. For more complicated network setups, please see the [documentation for the future package](https://cran.r-project.org/web/packages/future/vignettes/future-1-overview.html). 
