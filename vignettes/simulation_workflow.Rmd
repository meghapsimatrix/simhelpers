...
The code below specifies two design factors: `n`, which specifies the sample size per group assuming equal sample size and `mean_diff`, which denotes the mean difference between two groups on an outcome. These are the between simulation factors. The within simulation iteration factor is the t-test method with one assuming equal variance and one not assuming equal variance. 
