...
Here, we present a brief example on using `calc_absolute()` function from this package to calculate bias. For demonstration, we use the `welch_res` dataset included in the package containing results from an example simulation study comparing the heteroskedasticity-robust Welch t-test to the usual two-sample t-test assuming equal variances.
...
The conditions tested in this simulation include `n1` and `n2`, indicating sample size of the two groups, as well as `mean_diff`, indicating the true mean difference. Below we take the results and group the data by method, sample size for group 1, sample size for group 2 and the true mean difference. We then run the `calc_absolute()` function to calculate. The function returns a `tibble` containing absolute performance criteria and their corresponding MCSE.
