...
test_that("winsorization works for absolute performance measures", {
  ...
  res_wins <- calc_absolute(dat, x, true_param = mu, winsorize = winz)
  ...
  res_wins <- calc_absolute(dat, SE_sq, true_param = sigma_sq, winsorize = var_winz)
...
test_that("winsorization works for relative performance measures", {
  ...
  res_wins <- calc_relative(dat, x, true_param = mu, winsorize = winz)
  ...
  res_wins <- calc_relative(dat, SE_sq, true_param = sigma_sq, winsorize = var_winz)
...
test_that("winsorization works for coverage measures", {
  ...
  dat$hi <- dat$lo + dat$x
  res_wins <- calc_coverage(dat, lower_bound = lo, upper_bound = hi, true_param = mu, winsorize = winz)
...
test_that("winsorization works for relative variance measures", {
  ...
  res_wins <- calc_relative_var(dat, estimates = x, var_estimates = SE_sq, winsorize = winz, var_winsorize = var_winz)
